...
3. **Importance_score(重要度算出)**: コメント内容から'具体性'・'緊急性'・'共通性'を考慮して、1-10の重要度スコアを算出する
4. **Risk_level(危険度検知)**:コメント内容が何に言及しているのかを理解し、コメントの緊急性や危険性に応じて'high（緊急・危険）','medium（やや危険）','low（通常）'のいずれかに分類してください。
   *'high（緊急・危険）':講師への誹謗中傷、講義内容の誤りなど、至急運営が確認すべき内容。
   *'medium（やや緊急・やや危険）':講師や運営への批判など、出来るだけ早く確認すべき内容。
   *'low（通常）':上記のいずれにも当てはまらない内容
